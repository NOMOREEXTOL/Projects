...
class Main:
    ...
    def setup(self,filePath,HN,ON):
        ...
        with open(filePath,mode='r',encoding='utf-8') as f:
            interArray = [i.strip().split('')[:-1] for i in f.readlines()]
            self.output_label = np.array([i.strip().split('')[-1] for i in f.readlines()]).T
        self.input_values = np.array(interArray).T

        self.input_n = len(interArray[0])
        self.hidden_n = HN
        self.output_n = ON
        self.X = len(interArray)
        ...
        for h in range(self.hidden_n):
            for o in range(self.output_n):
                self.output_weighs[o][h] = Tools.rand(-1,1)
...
if __name__ == "__main__":
    f = open(r'C:\Users\xiaomi\Desktop\test.txt', mode='r', encoding='utf-8')
    interArray = [i.strip().split()[:-1] for i in f.readlines()]
    print(np.array(interArray).T)
